...
datapath = "../../../data/GroupF/" 
filename = sys.argv[1]
#filename = "SN20USEHB0000021.txt"
...
for i in range(0,len(cold_hybrids)):
    fig = plotRCClassified(cold_hybrids[i], variable="innse", showStream0=read_stream0, showStream1=read_stream1)
    #fig = plotRCClassified(cold_hybrids[-3], variable="innse", showStream0=read_stream0, showStream1=read_stream1)

    plt.ylabel("Gain")
    plt.xlabel("# of channels")
    plt.title(i)
    plt.legend()
    plt.savefig("spectrum"+str(i)+".png")
    plt.show()
